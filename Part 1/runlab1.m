...
for i = 1:length(x1)
   [posteriors_x,g_x]=lab1(x1(i),trainingSet,2);
end


f1=trainingSet(:,2);  % feature samples
f2=trainingSet(:,1);  % feature samples
...
syms th
px = 0.7*(1/(sqrt(2*pi*std11)))*exp(-.5*((th-m11)/std11)^2)-1.3*(1/(sqrt(2*pi*std12)))*exp(-.5*((th-m12)/std12)^2); %since p(w1)=p(w2) it cancels on each eq
Th1 = solve(px,th);
Th1 = double(Th1);
Th1 = (Th1(find(Th1>0)));
